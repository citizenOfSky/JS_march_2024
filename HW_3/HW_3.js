...
for (let i = 0; i < users.length; i++){
    const user = users[i];
    document.write(`<div>
                            <h3>${user.name} - ${user.username}: ${user.password}</h3>
                        </div>`)
}
...
for (let i =0; i <minerals.length; i++){
    ...
    document.write(`<div>
                            <h3>${mineral.id}. ${mineral.name}: ${mineral.chemical_formula}</h3>
                        </div>`)
}
...
// - За допомогою циклу while вивести в документ 20 блоків h1 c довільним текстом всередині.
{
    ...
    while (i < locations.length) {
        ...
        document.write(`<h1>${location.title} - ${location.type}</h1>`);
        i++;
    }
}
...
while (i < locations.length) {
    let location = locations[i];
    document.write(`<h1>${location.id}. ${location.title} - ${location.type}</h1>`);
    i++;
}
